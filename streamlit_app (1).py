...
sections = {
    "About": {
        "content": \"\"\"Experienced Business Analyst and aspiring Project Manager with a proven record of leading complex projects, performing gap analyses, and implementing strategic solutions to[...]\"\"\"
    },
    "Experience": {
        "content": [
            {
...
# Save file to disk for download (robust, with fallback to in-memory download)
import os
...
written = False
try:
    # Ensure parent directory exists (prevents FileNotFoundError)
    ...
    # Do not crash the app — fall back to in-memory download
    st.warning(f"Could not write file to {file_path}. Offering download directly. Error: {e}")

# Offer download button. If file was written, read bytes from disk (so users can inspect path). Otherwise use content in-memory.
if written:
    st.success(f"Saved app to {file_path}")
    try:
        with open(file_path, "rb") as f:
            file_bytes = f.read()
        st.download_button("Download app.py", data=file_bytes, file_name="devarsh_portfolio_app.py", mime="text/x-python")
    except Exception as e:
        # If reading fails, fall back to in-memory
        st.warning(f"Could not read the saved file for download. Offering in-memory download. Error: {e}")
        st.download_button("Download app.py", data=app_content.encode("utf-8"), file_name="devarsh_portfolio_app.py", mime="text/x-python")
else:
    st.download_button("Download app.py", data=app_content.encode("utf-8"), file_name="devarsh_portfolio_app.py", mime="text/x-python")
